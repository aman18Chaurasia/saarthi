"""Prompt assembly for the gold_loan dialog nodes.

Loads gold_loan.yaml once at module import and builds per-node system prompts.
The YAML is the single source of truth for all agent script text (ADR 0002 §5).

YAML search order:
  1. GOLD_LOAN_YAML env var (absolute path)
  2. packages/scripts/products/gold_loan.yaml relative to repo root
...
_SLOT_GUIDANCE: dict[str, str] = {
    ...
    "qualify_followup": (
        "Extract: gold_weight_grams (number), gold_purity_karats (integer).\n\n"
        "EXAMPLES:\n"
        "Customer: '25 grams, 22 karat' → {\"gold_weight_grams\": 25, \"gold_purity_karats\": 22}, intent: provide_value\n"
        "Customer: 'Around 40 gram sona hai' → {\"gold_weight_grams\": 40}, intent: provide_value\n"
        "Customer: '18 carat jewellery, about 15 grams' → {\"gold_weight_grams\": 15, \"gold_purity_karats\": 18}, intent: provide_value\n"
        "Customer: '22 karat hai' → {\"gold_purity_karats\": 22}, intent: provide_value\n\n"
        "RULES: Extract any explicit gold weight and purity. Use grams for weight and karat number for purity."
    ),
    "consent": (
...
_SYSTEM_TEMPLATE = """\
You are {agent_name} from {lender_name}, conducting a gold loan outbound call.
...
6. MIXED LANGUAGE is normal — Hindi/Urdu/English — extract info anyway
7. Add natural fillers: "Dekhiye", "Acha suniye", "Haan ji"
8. If frustrated, be empathetic: "Main samajhta hoon, tension mat lijiye"

COMMON QUESTIONS & ANSWERS:
Q: Interest rate kya hai? -> "Gold loan pe 9-12% per annum, gold ke value ke basis pe"
Q: Kitna loan milega? -> "Gold ke value ka 75% tak loan mil sakta hai"
Q: Documents kya chahiye? -> "Bas Aadhaar/PAN aur gold jewellery. Bahut simple process hai"
Q: Kitne din mein milega? -> "Same day disbursement possible hai — 1-2 ghante mein"
Q: Kya karat gold chahiye? -> "18 karat ya zyada. 22 karat standard hota hai"
