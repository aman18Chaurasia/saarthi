"""Prompt assembly for the education_loan dialog nodes.

Loads education_loan.yaml once at module import and builds per-node system prompts.
The YAML is the single source of truth for all agent script text (ADR 0002 §5).

YAML search order:
  1. EDUCATION_LOAN_YAML env var (absolute path)
  2. packages/scripts/products/education_loan.yaml relative to repo root
...
def _load_yaml() -> dict[str, Any]:
    env_path = os.environ.get("EDUCATION_LOAN_YAML")
    if env_path:
        return yaml.safe_load(Path(env_path).read_text(encoding="utf-8"))

    # packages/dialog/dialog/education_loan/prompts.py
    # parents[3] = packages/dialog → parents[3].parent = packages
    repo_packages = Path(__file__).parents[3]
    default_path = repo_packages / "scripts" / "products" / "education_loan.yaml"
    if default_path.exists():
        return yaml.safe_load(default_path.read_text(encoding="utf-8"))

    raise FileNotFoundError(
        f"education_loan.yaml not found. Set EDUCATION_LOAN_YAML env var or "
