...
Loads commercial_vehicle_loan.yaml once at module import and builds per-node system prompts.
...
YAML search order:
  ...
  2. packages/scripts/products/commercial_vehicle_loan.yaml relative to repo root
...
def _load_yaml() -> dict[str, Any]:
    ...
    default_path = repo_packages / "scripts" / "products" / "commercial_vehicle_loan.yaml"
    if default_path.exists():
        return yaml.safe_load(default_path.read_text(encoding="utf-8"))

    raise FileNotFoundError(
        f"commercial_vehicle_loan.yaml not found. Set COMMERCIAL_VEHICLE_YAML env var or "
...
_NODE_SCRIPTS: dict[str, str] = {
    ...
    "qualify_followup": (
        "Extract: vehicle_type (string), vehicle_price_inr (integer).\n\n"
        "EXAMPLES:\n"
        "Customer: 'Truck chahiye, around 18 lakh' → {\"vehicle_type\": \"truck\", \"vehicle_price_inr\": 1800000}, intent: provide_value\n"
        "Customer: 'Tempo dekh raha hoon, 9 lakh ka' → {\"vehicle_type\": \"tempo\", \"vehicle_price_inr\": 900000}, intent: provide_value\n"
        "Customer: 'Pickup vehicle, budget 12 lakh' → {\"vehicle_type\": \"pickup\", \"vehicle_price_inr\": 1200000}, intent: provide_value\n"
        "Customer: 'Bus chahiye' → {\"vehicle_type\": \"bus\"}, intent: provide_value\n\n"
        "RULES: Extract vehicle type from the customer words and convert any stated budget into INR integer."
    ),
...
_SYSTEM_TEMPLATE = """\
You are {agent_name} from {lender_name}, conducting a commercial vehicle loan outbound call.
...
Q: Interest rate? -> "Commercial vehicle loan pe 10-14% per annum, vehicle type pe depend karta hai"
Q: Down payment? -> "Vehicle price ka 15-25% down payment chahiye"
Q: Documents? -> "Driving license, income proof, vehicle quotation chahiye"
